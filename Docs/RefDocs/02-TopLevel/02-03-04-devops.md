...
## Device crash and usage analytics
...
We'd like to know what our users are doing with the app. HockeyApp gives us a lot of that data straight from the devices; but we can also code up specialized measurements in the server, and send them as telemetry to Application Insights. 

For example, we can track the numbers of users achieving different levels of competence, and have those segmented and charted on the dashboard.

### Display

Application Insights has its own good dashboard and charting features, but it can also output to Power BI or other tools. This lets you combine server analytics with HockeyApp and other monitoring data.

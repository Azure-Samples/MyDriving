...
- IoT devices used in {Token:ProjectName}
- The field gateway used in {Token:ProjectName}
- How data gets from a device to the backend (communication protocols)
- Communication between the OBD device and the field gateway
...
Many IoT solutions include a *field gateway* that sits between your IoT devices and the service they communicate with. A field gateway is typically located close to your devices. Field gateways are often used to enable connectivity and protocol translation for devices that either cannot or should not connect directly to the internet. An example of a device that cannot connect to the internet is one that only supports the Bluetooth communication protocol. An example of a device that should not connect to the internet is one that is unable to use a secure protocol when it connects to the cloud-based service.
...
The [TODO:appname] solution uses On-board Diagnostics (OBD) data from your car to [TODO:expand this]. A device in your car collects the data and sends to the solution back end for analysis. Modern cars have a standard OBD-II Data Link Connector somewhere in the cabin that enables you plug in a OBD-II compliant dongle that can read the OBD data generated by the car and make it available to other local devices over USB, Bluetooth or a local WiFi network.

### What is OBD and does it do?

Depending on the make, model, and age of your car, you can collect various types of data through the ODB connector:
...
> Note: In addition to retrieving potentially sensitive and personally identifiable information from the car, an OBD dongle plugged into the OBD-II Data Link Connector can also make changes to the firmware in the car [TODO:what more should we say here?].
...
All car manufacturers do not make the same types of data available over OBD-II. The [TODO:appname] makes use of the following common OBD data types collected from a car:
...
During the development of the [TODO:appname] we used several different OBD-II dongles to collect data from the car. Most OBD dongles cannot connect directly to a cloud-based back end, therefore they must connect to local field gateway that can forward the telemetry data. This table lists the dongles we used and why, however there are many more to choose from that are compliant with the OBD-II standards.
...
Most OBD dongles are not able to connect directly to a cloud-hosted service, but are capable of making a local network connection over Bluetooth or WiFi, or a physical connection using a USB cable. To send the data collected by the OBD dongle to the cloud, you need a device that can act as a field gateway in the solution.

### Decision point: why did we choose the phone

Most drivers will carry a mobile phone with them that is capable of connecting to an IoT Hub endpoint on the cloud over a public cellular network. Most smart phones are also capable of making ad-hoc local connections using Bluetooth or WiFi.

In the [TODO:appname] solution, the phone is not just a protocol converter that translates the OBD data received over a local connection into a message that is sent to IoT Hub using the AMQPS [TODO:check this is the protocol in use].
...
[TODO: add mention of library support for AMQP]
